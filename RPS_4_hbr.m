...
%% Scan for all subjects
  sourceList    = dir([srcPath, 'RPS_*_C_hbr.mat']);
  ...
  for ii=1:1:numOfSources
    numOfPart(ii)  = sscanf(sourceList{ii}, ...
                    strcat('RPS_%d_C_hbr.mat'));
  end
  %%
  for i=numOfPart
      
    
    filename    = sprintf(['RPS_%02d_C_hbr'], i);
...
%% Scan for all subjects
  sourceList    = dir([srcPath, 'RPS_*_FP_hbr.mat']);
  sourceList    = struct2cell(sourceList);
  sourceList    = sourceList(1,:);
  numOfSources  = length(sourceList);
  PartList       = zeros(1, numOfSources);
  for ii=1:1:numOfSources
    numOfPart(ii)  = sscanf(sourceList{ii}, ...
                    strcat('RPS_%d_FP_hbr.mat'));
  end
  %%
  for i=numOfPart
      
    
    filename    = sprintf(['RPS_%02d_FP_hbr'], i);
...
%% Scan for all subjects
  sourceList    = dir([srcPath, 'RPS_*_PD_hbr.mat']);
  sourceList    = struct2cell(sourceList);
  sourceList    = sourceList(1,:);
  numOfSources  = length(sourceList);
  PartList       = zeros(1, numOfSources);
  for ii=1:1:numOfSources
    numOfPart(ii)  = sscanf(sourceList{ii}, ...
                    strcat('RPS_%d_PD_hbr.mat'));
  end
  %%
  for i=numOfPart
      
    
    filename    = sprintf(['RPS_%02d_PD_hbr'], i);
...
%% Scan for all subjects
  sourceList    = dir([srcPath, 'RPS_*_PS_hbr.mat']);
  sourceList    = struct2cell(sourceList);
  sourceList    = sourceList(1,:);
  numOfSources  = length(sourceList);
  PartList       = zeros(1, numOfSources);
  for ii=1:1:numOfSources
    numOfPart(ii)  = sscanf(sourceList{ii}, ...
                    strcat('RPS_%d_PS_hbr.mat'));
  end
  %%
  for i=numOfPart
      
    
    filename    = sprintf(['RPS_%02d_PS_hbr'], i);
...
rest_coher=mean(rest_coherences,2);
  ...
  dlm_write("C:\Users\Trinh Nguyen\Documents\ucloud\RPS\Statistics\RPS_wtc_hbr.csv",wtc);
%% %%%%%%% Extract GLM data %%%%%%%%%%%%%%%%
% %%%%%%%%%%%%%%%
% clear all 
% 
